...
export function getUserProgress(): UserProgress | null {
  try {
    const data = storage.getString(STORAGE_KEYS.USER_PROGRESS);
    if (!data) return null;
...
export function saveUserProgress(progress: UserProgress): void {
  try {
    storage.set(STORAGE_KEYS.USER_PROGRESS, JSON.stringify(progress));
  } catch (error) {
...
export function getWorkoutHistory(): WorkoutHistory {
  try {
    const data = storage.getString(STORAGE_KEYS.WORKOUT_HISTORY);
...
export function addCompletedWorkout(workout: CompletedWorkout): void {
  try {
    ...
    storage.set(STORAGE_KEYS.WORKOUT_HISTORY, JSON.stringify({ workouts: filteredWorkouts }));
  } catch (error) {
...
export function getActiveWorkout(): ActiveWorkout | null {
  try {
    const data = storage.getString(STORAGE_KEYS.ACTIVE_WORKOUT);
    if (!data) return null;
...
export function saveActiveWorkout(workout: ActiveWorkout): void {
  try {
    storage.set(STORAGE_KEYS.ACTIVE_WORKOUT, JSON.stringify(workout));
  } catch (error) {
...
export function clearActiveWorkout(): void {
  try {
    storage.delete(STORAGE_KEYS.ACTIVE_WORKOUT);
  } catch (error) {
