  ...
  return (
    <View style={[styles.container, getCardStyle()]}>
      <View style={styles.header}>
        <Text style={[styles.setNumber, getTextStyle()]}>Set {setNumber}</Text>
        {status === "completed" && <Text style={styles.checkmark}>✓</Text>}
      </View>

      <Text style={[styles.reps, getTextStyle()]}>{displayReps()}</Text>
      ...
      {status === "active" && onComplete && (
        <Pressable onPress={onComplete} style={({ pressed }) => [styles.button, pressed && styles.buttonPressed]}>
